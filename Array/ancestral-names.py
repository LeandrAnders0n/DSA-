...
# value to indicate subtraction. For example, the letter I represents the number 1, and represents 5. Reason through
# the formation of 1 to 10 below, and see how it is applied in the following lines.
# • L I M. /V. V, WI, wiI, vI. IX and Xrepresent 1 through 10.
# • XX XOX XL and L are 20. 30. 40 and 50.
# • For any other two-digit number « 50 concatenate the Roman numerals) that represent its multiples of ten with the
# Roman numerals) for its values « 10. For example, 43 is 40 + 3= "XL*.
# Given a list of strings comprised of a name and a Roman numeral, sort the list first by name, then by decimal value of
# the Roman numeral.
# For example, If you are given the names [Steven XL Steven XVI, David IX, Mary XV, Mary XIII, Mary XX] the result of the
# sort is [David IX, Mary XIII, Mary XV, Mary XX, Steven XVI, Steven XL]. The result with Roman numerals is the expected
